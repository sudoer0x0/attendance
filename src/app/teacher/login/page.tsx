  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (data.requiresSetup) {
        router.push(`/login/setup?token=${encodeURIComponent(data.setupToken)}`);
        return;
      }
    ...
    <div className="flex min-h-dvh items-center justify-center bg-[var(--color-surface-subtle)] px-4">
      <div className="w-full max-w-[380px]">
        <div className="mb-8 text-center">
          <Badge>Teaching &amp; Academic Staff</Badge>
          ...
          <p className="mt-1 text-[13.5px] text-[var(--color-ink-subtle)]">
            {stage === "credentials" ? "Sign in to launch your class attendance sessions" : "Enter your authenticator code"}
          </p>
        </div>
        ...
        >
          {stage === "credentials" ? (
            ...
            <div className="flex flex-col gap-4">
              <Input
                ...
                hint="From your authenticator app. Lost access? Contact your department."
              />
            </div>
          )}
          ...
          <Button type="submit" className="mt-5 w-full" loading={loading}>
            {stage === "credentials" ? "Continue" : "Sign in"}
          </Button>
